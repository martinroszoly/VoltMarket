...
Object.assign(translations.hu, {
  ...
  techBonusEyebrow:'INGYENES TECH BÓNUSZ',techBonusTitle:'Tech jutalomsor',techBonusSubtitle:'Tizenöt percenként aktiválhatod a következő, előre látható játékbeli bónuszt.',techBonusClaim:'JUTALOM AKTIVÁLÁSA',techBonusReadyText:'A következő jutalom előre látható és tizenöt percenként aktiválható.',techBonusRewards:'20 ELÉRHETŐ JUTALOM',techBonusNoStake:'Ingyenes · nincs tét · a termékjutalmak prémium minőségűek',rewardUnlocked:'JUTALOM FELOLDVA',nextGuaranteedReward:'KÖVETKEZŐ BIZTOS JUTALOM',deterministicRewardNote:'Előre meghatározott sorrend · nincs véletlen · nincs tét',
...
Object.assign(translations.en, {
  ...
  techBonusEyebrow:'FREE TECH BONUS',techBonusTitle:'Tech reward sequence',techBonusSubtitle:'Activate the next visible in-game bonus every fifteen minutes.',techBonusClaim:'ACTIVATE REWARD',techBonusReadyText:'The next reward is visible in advance and can be activated every fifteen minutes.',techBonusRewards:'20 AVAILABLE REWARDS',techBonusNoStake:'Free · no stake · product rewards are premium quality',rewardUnlocked:'REWARD UNLOCKED',nextGuaranteedReward:'NEXT GUARANTEED REWARD',deterministicRewardNote:'Preset sequence · no randomness · no stake',
...
[
  {id:'dronefix',name:'Hibás AeroVolt drón',nameEn:'Faulty AeroVolt drone',completedName:'Megjavított AeroVolt drón',completedNameEn:'Repaired AeroVolt drone',icon:'✥',buy:145000,sell:720000,parts:['flightController','droneMotor','camera','battery'],fault:'Instabil repülésvezérlés és sérült kamerarendszer.',faultEn:'Unstable flight control and a damaged camera system.',level:50,premiumFloor:true,shelf:'drone'},
  {id:'mixedrealityfix',name:'Hibás Nova XR rendszer',nameEn:'Faulty Nova XR system',completedName:'Megjavított Nova XR rendszer',completedNameEn:'Repaired Nova XR system',icon:'◈',buy:175000,sell:840000,parts:['vrDisplay','motion','lens','lidar'],fault:'Térérzékelési és optikai kalibrációs hibák.',faultEn:'Spatial tracking and optical calibration faults.',level:50,premiumFloor:true,shelf:'xr'},
  {id:'robotfix',name:'Hibás Vector szervizrobot',nameEn:'Faulty Vector service robot',completedName:'Megjavított Vector szervizrobot',completedNameEn:'Repaired Vector service robot',icon:'✦',buy:210000,sell:980000,parts:['cpu','robotDrive','lidar','battery'],fault:'Meghajtási és navigációs rendszerhiba.',faultEn:'Drive and navigation system failure.',level:50,premiumFloor:true,shelf:'robotics'}
...
deliverOrder = function (index) {
  ...
  const gainedXp = grantXp(order.xp);
  state.balance += order.reward;
  addHistory(`${vmText('Ügyfélrendelés','Customer order')}: ${vmBlueprintName(blueprint(item.type))} (+${gainedXp} XP)`, order.reward);
  workshop.finished.splice(index,1);
  workshop.activeOrders.splice(orderIndex,1);
  vmCommit(vmText(`Megrendelés átadva: +${fmt(order.reward)} CR · +${gainedXp} XP`,`Order delivered: +${fmt(order.reward)} CR · +${gainedXp} XP`));
};
...
renderCustomerMarket = function () {
  ...
  const active = workshop.activeOrders.map(order => {
    ...
    return `<article class="offer-card compact active-order"><img class="customer-avatar" src="${p.avatar}" alt="${p.customer}"><span class="category">${vmText('ELFOGADOTT','ACCEPTED')}</span><h3>${p.customer}: ${vmBlueprintName(b)}</h3><p>${vmText('Kért minőség','Requested quality')}: <strong>${vmQualityName(order.quality)}</strong></p><div class="offer-reward">${fmt(order.reward)} CR · +${boostXpAmount(order.xp)} XP</div><small>${vmText('Nincs teljesítési határidő.','No completion deadline.')}</small></article>`;
  }).join('');
  ...
  if (marketEl.dataset.renderKey !== marketKey) {
    ...
    marketEl.innerHTML = workshop.marketListings.length ? workshop.marketListings.map(item => {const b=blueprint(item.type);return `<article class="market-card"><span class="device-icon">${b.icon}</span><div><strong>${completedProductName(b)}</strong><small>${vmQualityName(item.quality||'normal')}</small><span class="market-searching">● ${vmText('Vevő keresése…','Finding a buyer…')}</span></div></article>`}).join('') : `<div class="mini-empty">${vmText('Nincs piacon lévő termék.','No products are listed.')}</div>`;
  }
...
function vmCheckEnterpriseCompletion() {
  ...
  const gainedXp=grantXp(c.xp);state.balance+=c.reward;addHistory(`${vmText('Vállalati szerződés','Corporate contract')}: ${c.company} (+${gainedXp} XP)`,c.reward);
  workshop.enterpriseContract=null;workshop.enterpriseNextAt=Date.now()+30000;
  toast(vmText(`Vállalati szerződés teljesítve: +${fmt(c.reward)} CR · +${gainedXp} XP`,`Corporate contract completed: +${fmt(c.reward)} CR · +${gainedXp} XP`));
}
...
renderEmployees=function(){vmBaseRenderEmployees();const summary=document.querySelector('#employeeSummary');if(summary){const baseXp=employees.filter(e=>workshop.employees.includes(e.id)).reduce((sum,e)=>sum+vmEmployeeXpRate(e),0),xp=baseXp?boostXpAmount(baseXp):0;summary.insertAdjacentHTML('beforeend',`<div><span>${vmText('CSAPAT XP','TEAM XP')}</span><strong>+${xp} XP / 60 ${vmText('mp','sec')}</strong></div>`)}renderServiceCenter()};
